...
<head>
    <title>Title</title>
    <link rel="stylesheet" href="css02/batteryfee.css">
</head>
<body>
<%
    ...
    if (null==user){
        ...
        List<PeopleBattery> feelist = (List<PeopleBattery>) request.getAttribute("feelist");
%>
            ...
            <table border="1">
                ...
                <%
                    if (feelist != null && !feelist.isEmpty()) {
                        for (PeopleBattery fee: feelist) {
                %>
                <tr>
                    <td><%= fee.getBattery_id() %></td>
                    <td><%= fee.getPay_battery_time() %></td>
                    <td><%= fee.getPhone() %></td>
                    <td><%= fee.getBattery_fy() %></td>
                    <td><%= fee.getBattery_pay_zt() %></td>
                    <td>
                        <%
                            if (fee.getFinish() != null) {
                                out.print(fee.getFinish());
                            } else {
                                out.print("未支付");
                            }
                        %>
                    ...
                    <td>
                        <%
                            if (fee.getFinish() == null) {
                        %>
                        <form action="batterydetail?method=feefinish" method="post">
                            <input type="hidden" name="phone" value="<%= user.getPhone() %>">
                            <input type="hidden" name="id" value="<%= fee.getId() %>">
                            <input type="hidden" name="battery_id" value="<%= fee.getBattery_id() %>">
                            <input type="hidden" name="pay_battery_time" value="<%= fee.getPay_battery_time() %>">
