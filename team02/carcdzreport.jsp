...
<head>
    <title>Title</title>
    <link rel="stylesheet" href="css02/carcdzreport.css">
</head>
<body>
<%
    ...
    if (null==user){
        ...
        List<CarCdz> report = (List<CarCdz>) request.getAttribute("report");
%>
        ...
        <main>
            ...
            <table border="1">
                ...
                <tr>
                    <td><%= r.getCdz_id() %></td>
                    <td><%= r.getCph() %></td>
                    <td><%= r.getBegin_cd_time() %></td>
                    <td>
                        <%
                            if (r.getEnd_cd_time() != null) {
                                out.print(r.getEnd_cd_time());
                            } else {
                                out.print("充电中");
                            }
                        %>
                    ...
                    <td>
                        <%
                            if (r.getCd_times() != null) {
                                out.print(r.getCd_times()+"分钟");
                            } else {
                                out.print("结算中");
                            }
                        %>
                    ...
                    <td>
                        <%
                            if (r.getEnd_cd_time() == null) {
                        %>
                        <form action="cdz?method=end" method="post">
                            <input type="hidden" name="phone" value="<%= r.getPhone() %>">
                            <input type="hidden" name="cdz_id" value="<%= r.getCdz_id() %>">
                            <input type="hidden" name="cph" value="<%= r.getCph() %>">
                            <input type="hidden" name="begin_cd_time" value="<%= r.getBegin_cd_time() %>">
                            <input type="submit" value="结束充电">
                        </form>
                        <%
                            }
                        %>
