...
<%@ page import="team02.relation.PeopleBattery" %>
<%@ page import="team02.pojo.Car" %><%--
...
<head>
    <title>Title</title>
    <link rel="stylesheet" href="css02/admin_peoplelist.css">
</head>
<body>
<%
    ...
    if (null == user) {
        ...
        List<User02> peoplelist = (List<User02>) request.getAttribute("peoplelist");
%>
...
<div class="container">
    ...
    <div class="search-box">
        <form action="AdminUserServlet?method=search_people_key" method="post">
            <label for="search_input"></label><input type="text" name = "search_input" id="search_input" placeholder="输入身份证号码，学号，手机号，姓名，专业，年级，学校，账户状态等关键词...">
            ...
            <table class="battery-table">
                ...
                <%
                    if ( peoplelist != null && ! peoplelist.isEmpty()) {
                        for (User02 cb1 :  peoplelist) {
                %>
                <tr>
                    <td><%= cb1.getId()%></td>
                    <td><%= cb1.getName()%></td>
                    <td><%= cb1.getPhone()%></td>
                    <td><%= cb1.getPeople_registration()%></td>
                    <td><%= cb1.getStudent_id() %></td>
                    <td><%= cb1.getGrade()%></td>
                    <td><%= cb1.getProfession()%></td>
                    <td><%= cb1.getSchool()%></td>
                    <td><%= cb1.getAccounts()%></td>
                    <td><%= cb1.getPassword()%></td>
                    <td><%= cb1.getRole() %></td>
                    <td><%= cb1.getZt()%></td>
                    <td>
                        <%
                            if (cb1.getZt().equals("正常")){
                        %>
                        <form action="AdminUserServlet?method=black" method="post">
                            <input type="hidden" name="id" value="<%= cb1.getId()%>">
                            <input type="hidden" name="zt" value="黑名单">
