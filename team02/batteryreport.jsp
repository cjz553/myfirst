...
<head>
    <title>Title</title>
    <link rel="stylesheet" href="css02/batteryreport.css">
</head>
<body>
<%
    ...
    if (null==user){
        ...
        List<CarBattery2> reportlist = (List<CarBattery2>) request.getAttribute("reportlist");
%>
            ...
            <table border="1">
                ...
                <%
                    if (reportlist != null && !reportlist.isEmpty()) {
                        for (CarBattery2 report: reportlist) {
                %>
                <tr>
                    <td><%= report.getBattery_id() %></td>
                    <td><%= report.getCph() %></td>
                    <td><%= report.getChange_time() %></td>
                    <td><%= report.getPhone() %></td>
                    <td>
                        <%
                            if (report.getBack_time() != null) {
                                out.print(report.getBack_time());
                            } else {
                                out.print("未归还");
                            }
                        %>
