...
<head>
    <title>Title</title>
    <link rel="stylesheet" href="css02/admin_carlist.css">
</head>
<body>
<%
...
<div class="container">
    ...
    <div class="search-box">
        <form action="AdminCarServlet?method=search_car_key" method="post">
            <label for="search_input"></label><input type="text" name = "search_input" id="search_input" placeholder="输入车牌号，手机号，是否报废，品牌等关键词...">
        ...
        <main>
            ...
            <table class="battery-table">
                ...
                <tr>
                    ...
                    <td><%= cb1.getCzrs()%></td>
                    <td><%= cb1.getXh()%></td>
                    <td><%= cb1.getChanges()%></td>
                    <td>
                        <%
                            if (cb1.getChange_time()==null){
                                out.print("无改装记录");
                            }else {
                                out.print(cb1.getChange_time());
                            }
                        %>
                    </td>
                    <td><%= cb1.getCar_gistration() %></td>
                    <td><%= cb1.getBegin_id()%></td>
                    <td>
                        <%
                            if (cb1.getBaofei().equals("是")){
                                out.print("报废");
                            }
                            else{
                                out.print("正常");
                    ...
                    <td>
                        <%
                            if (cb1.getBaofei().equals("否")){
                        %>
                        <form action="AdminCarServlet?method=baofei" method="post">
                            <input type="hidden" name="cph" value="<%= cb1.getCph()%>">
                            <input type="hidden" name="baofei" value="是">
                        ...
                        <form action="AdminCarServlet?method=baofei" method="post">
                            <input type="hidden" name="cph" value="<%= cb1.getCph()%>">
                            <input type="hidden" name="baofei" value="否">
                            <input type="submit" value="取消报废">
                        </form>
                        <%
                            }
                        %>
