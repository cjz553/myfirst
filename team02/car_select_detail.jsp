...
<head>
    <title>Title</title>
    <link rel="stylesheet" href="css02/car_select_detail.css">
</head>
<body>
<%
...
<div class="container">
    ...
    <div class="main-content">
        <div class="container">
            <div class="info-title"><%= car.getCph() %>的详细信息</div>
            <ul class="info-list">
                <li><span>车牌号:</span><span><%= car.getCph() %></span></li>
                <li><span>品牌:</span><span><%= car.getPp() %></span></li>
                <li><span>车架号:</span><span><%= car.getCjh() %></span></li>
                <li><span>车型:</span><span><%= car.getCx() %></span></li>
                <li><span>载重量:</span><span><%= car.getZzl() %></span></li>
                <li><span>最大乘坐人数:</span><span><%= car.getCzrs() %></span></li>
                <li><span>兼容电池型号:</span><span><%= car.getXh() %></span></li>
                <li><span>是否改装:</span><span><%= car.getChanges() %></span></li>
                <li><span>改装时间:</span><span>
                         <%
                             if (car.getChange_time()==null){
                                 out.print("无改装记录");
                             }else {
                                 out.print(car.getChange_time());
                             }
                         %>
                </span></li>
                <li><span>车辆登记时间:</span><span><%= car.getCar_gistration() %></span></li>
                <li><span>初始电池编号:</span><span><%= car.getBegin_id() %></span></li>
            </ul>
        </div>
