...
<%@ page import="team02.relation.AreaCar2" %>
...
<head>
    <title>Title</title>
    <link rel="stylesheet" href="css02/admin_carwtlist.css">
</head>
<body>
<%
    ...
    if (null == user) {
        ...
        List<AreaCar> carwtlist = (List<AreaCar>) request.getAttribute("carwtlist");
%>
...
<div class="container">
    ...
    <div class="search-box">
        <form action="AdminCarServlet?method=search1" method="post">
            <label for="search_input"></label><input type="text" name = "search_input" id="search_input" placeholder="输入存放地编号，车牌号等关键词...">
    ...
    <div class="main-content">
        <main>
            ...
            <table class="battery-table">
                ...
                <tbody>
                <%
                    if ( carwtlist != null && ! carwtlist.isEmpty()) {
                        for (AreaCar cb1 :  carwtlist) {
                %>
                <tr>
                    <td><%= cb1.getCar_area_id()%></td>
                    <td><%= cb1.getCph() %></td>
                    <td><%= cb1.getWt()%></td>
                    <td><%= cb1.getLa()%></td>
                    <td><%= cb1.getLo()%></td>
                    <td><%= cb1.getBegin_time()%></td>
                    <td>
                        <%
                            if (cb1.getEnd_time()==null){
                                out.print("停放中");
                            }
                            else{
                                out.print(cb1.getEnd_time());
                            }
                        %>
                    </td>
                    <td><%= cb1.getPhone()%></td>
