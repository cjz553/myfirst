...
<%@ page import="team02.relation.CarPeople" %>
...
<head>
    <title>Title</title>
    <link rel="stylesheet" href="css02/car_baofei.css">
</head>
<body>
<%
    ...
    if (null==user){
        ...
        List<CarPeople> carlist = (List<CarPeople>) request.getAttribute("carlist");
        List<CarPeople> baofeilist = (List<CarPeople>) request.getAttribute("baofeilist");
%>
    ...
    <div class="main-content">
        <main>
            ...
            <table>
                ...
                <%
                    if (carlist != null && !carlist.isEmpty()) {
                        for (CarPeople carpeople: carlist) {
                %>
                <tr>
                    <td><%= carpeople.getCph() %></td>
                    <td><%= carpeople.getBegin_have_time() %></td>
                    <td>
                        <form action="car?method=baofei2" method="post">
                            <input type="hidden" name="cph" value="<%= carpeople.getCph() %>">
                            <input type="hidden" name="phone" value="<%= user.getPhone() %>">
                            <input type="submit" id="fixed-input" value="强制报废">
                        </form>
                    </td>
                </tr>
                <%
            ...
            <table>
                ...
                <tbody>
                <%
                    if (baofeilist != null && !baofeilist.isEmpty()) {
                        for (CarPeople carpeople: baofeilist) {
                %>
                <tr>
                    <td><%= carpeople.getCph() %></td>
                    <td><%= carpeople.getBegin_have_time() %></td>
