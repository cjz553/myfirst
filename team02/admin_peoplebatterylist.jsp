...
<head>
    <title>Title</title>
    <link rel="stylesheet" href="css02/admin_peoplebatterylist.css">
</head>
<body>
<%
    ...
    if (null == user) {
        ...
        List<PeopleBattery> peoplebatterylist = (List<PeopleBattery>) request.getAttribute("peoplebatterylist");
%>
...
<div class="container">
    ...
    <div class="search-box">
        <form action="AdminUserServlet?method=search" method="post">
            <label for="search_input"></label><input type="text" name = "search_input" id="search_input" placeholder="输入电池编号，手机号，支付状态等关键词...">
    ...
    <div class="main-content">
        <main>
            ...
            <table class="battery-table">
                ...
                <tbody>
                <%
                    if ( peoplebatterylist != null && ! peoplebatterylist.isEmpty()) {
                        for (PeopleBattery cb1 :  peoplebatterylist) {
                %>
                <tr>
                    <td><%= cb1.getBattery_id()%></td>
                    <td><%= cb1.getPhone()%></td>
                    <td><%= cb1.getPay_battery_time()%></td>
                    <td><%= cb1.getBattery_fy()%></td>
                    <td><%= cb1.getBattery_pay_zt() %></td>
                    <td>
                        <%
                            if (cb1.getFinish()==null){
                                out.print("支付未完成");
                            }else {
                                out.print(cb1.getFinish());
                            }
                        %>
